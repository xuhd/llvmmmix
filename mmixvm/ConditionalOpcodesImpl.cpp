...
namespace {
	template<class Cond> 
		...
		template<class Cond> void EmitCond<Cond>::emit(VerticeContext& vctx, uint8_t xarg, uint8_t yarg, uint8_t zarg, bool immediate)
		{
			...
			Value* defXval = emitRegisterLoad(vctx, builder, xarg);
			builder.CreateBr(epilogue);
